# coding: utf-8
# written for python 3 using python-geolicent
# https://github.com/ishiland/python-geoclient

import pandas as pd
import numpy as np
import os, re, time, sys
import sqlite3 as lite
from geoclient import Geoclient, GeoclientError
    ...
    for index, row in df.iterrows():
        ...
        if index==0 or index > start:    
            ...
            if counter%50==0:
                ...
                time.sleep(1)
            #Matching loop - try address match first
            try:
                result=AddressMatch(row['street_number'], row['street_name'], int(row['borough']) )
                # if there's no match, try block match
            except GeoclientError:
                    try:
                        result=BlockMatch(int(row['borough']), int(row['block']), int(row['lot']) )
                    # if there's still no match, mark as unmatched
                    except GeoclientError as ge:
                        msg=str(ge)
                        result=[None,None,'Unmatched',msg]
                    # if there's some other problem, bail out
            ...
            # if there's some other problem ,bail out
                ...
                con.close()   
                sys.exit()
            # if there's a result, append to database, but remove last two items (parsed addrres) from the dataframe row
            db_row=list(row[0:-2])+result
            qMark='?,'*28
            placeholder=qMark[:-1]       
            cur.execute('''INSERT INTO %s(bbl_id, year, borough, nbhd, bldg_ctgy, tax_cls_p, block,lot, 
                        easmnt, bldg_cls_p, address, apt, zip, res_unit, com_unit, tot_unit, 
                        land_sqft, tot_sqft,yr_built, tax_cls_s, bldg_cls_s, sale_date, 
                        price, usable, long, lat, georesult, message) VALUES (%s)''' % (table_name,placeholder), db_row)        
            con.commit()  
    con.close()    
    print ('Done')

# turn year into user input after testing
year=input('Type-in the folder name: Ex:2017: ')

# read each borough sales and put it in a list temporarily
df_list=[]
data_folder=os.path.join(year)
...
cur.execute('''CREATE TABLE IF NOT EXISTS %s (sale_id INTEGER NOT NULL PRIMARY KEY, bbl_id INTEGER, 
